      ...
      <aside className="w-80 bg-[#0d1117] border-r border-[#30363d] p-5 flex flex-col justify-between overflow-y-auto flex-shrink-0 select-none">
        <div className="space-y-5">
          <div className="flex items-center gap-3 border-b border-[#21262d] pb-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-white">OctoVibe Workspace</h1>
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Visual Theme Palette</label>
              <div className="grid grid-cols-2 gap-1.5">
                {themesList.map(t => (
                  <button key={t.id} onClick={() => setActiveTheme(t.id)} className={`px-2 py-1 rounded text-[10px] font-bold border transition-all ${activeTheme === t.id ? 'border-white text-white' : 'border-transparent text-gray-500 bg-[#161b22]'}`} style={{ backgroundColor: activeTheme === t.id ? p.primaryColor : '' }}>{t.name.split(' ')[0]}</button>
                ))}
              </div>
            </div>
            ...
            <div className="border-t border-[#21262d] pt-3 space-y-2">
              <label className="block text-[10px] font-bold uppercase text-gray-400">Contribution Grid Generator Settings</label>
              ...
              <input type="text" value={artText} onChange={e => setArtText(e.target.value.toUpperCase())} placeholder="Grid Typographic Characters (A-Z)" className="w-full bg-[#161b22] border border-[#30363d] rounded-md px-3 py-1.5 text-xs text-white outline-none" />
              <div className="grid grid-cols-2 gap-1.5">
                <select value={artStyle} onChange={e => setArtStyle(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded-md p-1 text-[10px] text-white outline-none"><option value="flat">2D Flat</option><option value="3d">3D Glass</option></select>
                <select value={artBg} onChange={e => setArtBg(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded-md p-1 text-[10px] text-white outline-none"><option value="0">Shade 0</option><option value="1">Shade 1</option><option value="2">Shade 2</option><option value="random">Scramble</option></select>
              </div>
              <button onClick={saveArtImage} className="w-full bg-[#238636] hover:bg-[#2ea043] text-white font-bold py-1.5 rounded text-xs transition-colors"><i className="fas fa-file-image mr-1.5"></i> Export Pattern PNG</button>
            </div>
            ...
            <div className="border-t border-[#21262d] pt-3">
              ...
              <div className="grid grid-cols-2 gap-1">
                {Object.keys(visible).map(key => (
                  <button key={key} onClick={() => setVisible(p => ({ ...p, [key]: !p[key] }))} className={`py-1 rounded text-[9px] font-bold uppercase border transition-all ${visible[key] ? 'bg-emerald-950/40 text-emerald-400 border-emerald-500/20' : 'bg-transparent text-gray-600 border-transparent'}`}>{key}</button>
                ))}
              </div>
            </div>
          </div>
        </div>
        <div className="text-[10px] text-gray-500 font-bold border-t border-[#21262d] pt-3">OctoVibe Engine • Production Mirror</div>
      </aside>

      {/* STACKABLE CENTRAL PREVIEW CANVAS TIMELINE */}
      <main className="flex-1 overflow-y-auto p-10 space-y-6">
        
        <div className="p-8 rounded-xl border" style={{ backgroundColor: p.background, borderColor: p.cardBorder }}>
          <div className="w-full space-y-8 max-w-[740px] mx-auto">
            
            {/* MODULE 1: PROFILE HERO HEADER CARD SHELLS */}
            {visible.hero && (
              <div className="w-full">
                {heroLayout === 'minimalist' && (
                  <div className="text-center py-2">
                    ...
                    <p className="text-xs text-gray-300 mt-2 font-medium leading-relaxed">{profile.bio}</p>
                  </div>
                )}
                {heroLayout === 'terminal' && (
            ...
            {/* MODULE 2: REPOSITORIES AND STARS DATA CARDS COUNTERS */}
            {visible.metrics && (
              <div className="grid grid-cols-4 gap-4 w-full">
                {[
                  ...
                  <div key={idx} className="p-4 rounded-xl border" style={{ backgroundColor: p.cardBg, borderColor: p.cardBorder }}>
                    <div className="flex justify-between items-center text-[10px] font-bold text-gray-400 uppercase mb-1"><span>{s.label}</span><i className={`fas ${s.icon}`} style={{ color: p.primaryColor }} /></div>
            ...
            {/* MODULE 3: COMMIT TIMELINE WINDOW STREAKS */}
            {visible.streak && (
            ...
            {/* MODULE 4: TECH LANGUAGE PIEPELINE CHARTS */}
            {visible.languages && (
              <div className="p-5 rounded-xl border bg-black/10 w-full" style={{ borderColor: p.cardBorder }}>
                ...
                <div className="grid grid-cols-4 gap-3 mt-4">
                  {profile.topLanguages.map((l, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-xs font-bold text-white">
                      <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: l.color }} /><span>{l.name}</span><span className="text-gray-500 font-mono ml-auto text-[10px]">{l.percentage}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* MODULE 5: 14+ ACHIEVEMENTS TROPHIES GRID FRAME */}
            {visible.trophies && (
              <div className="p-5 rounded-xl border bg-black/10 w-full" style={{ borderColor: p.cardBorder }}>
            ...
            {/* MODULE 6: RESPONSIVE FLUID PATTERN CODE ART CANVAS VIEWS */}
            {visible.art && (
              <div className="w-full overflow-hidden">
                <div id={artStyle === 'flat' ? 'flatCanvas' : 'threeDCanvas'} className="p-5 rounded-xl border shadow-xl bg-[#0d1117] w-full border-[#30363d]">
                  ...
                  {/* Fluid Adaptive Canvas Wrapper Framework */}
                  <div className="w-full overflow-x-auto">
                    <div className="w-full flex flex-col gap-1 select-none">
                      <div className="flex h-4 relative text-[8px] text-gray-500 font-bold mb-1 pl-7 w-full">
                        {Array.from({ length: totalCols }).map((_, i) => i % 4 === 0 && <span key={i} className="absolute" style={{ left: `${(i / totalCols) * 100}%` }}>M</span>)}
                      </div>
                      <div className="flex gap-1.5 w-full">
                        <div className="text-[8px] text-gray-500 font-bold h-[78px] flex flex-col justify-between pr-1 select-none"><span>M</span><span>W</span><span>F</span></div>
                  ...
                  <div className="mt-4 flex items-center justify-between text-[10px] text-gray-400">
                    <span><b>{artCommits}</b> commits required to map this blueprint text matrix array stack.</span>
        ...
        {/* COMPREHENSIVE PRODUCTION MARKDOWN SNIPPET CODE REGISTRY ROW */}
