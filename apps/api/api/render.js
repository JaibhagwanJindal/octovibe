...
export default async function handler(req, res) {
  ...
  // VIEW MODE D: LIVE GRAPHQL STREAK PIPELINE — uses dynamic profile fields
  else if (view === 'streak') {
  ...
  // VIEW MODE E: CUSTOM TYPOGRAPHIC CONTRIBUTION GRID ART WITH MONTH HEADERS
  else if (view === 'art') {
    ...
    let monthHeaders = MONS.map((m, idx) => {
      let x = (idx * (svgWidth - 100) / 12) + 50;
      return `<text x="${x}" y="50" fill="${p.textTertiary}" font-family="sans-serif" font-size="9" font-weight="bold">${m}</text>`;
    }).join('');
